...
import json
import z3
from crossword_grid import CROSSWORD_GRID
...
class Solve():
	...
	def fetch_clues(self):
		"""Fetches the candidates for the clues  present in "candidates.json"
		"""
		candidates = dict()
	...
	def convert_clues_code(self):
		...
		for clue in clues:
			for guess in clues[clue]:
				try:
					clues_ord[clue].append([ord(ch) for ch in guess])
				except:
					clues_ord[clue] = [[ord(ch) for ch in guess]]
...
if __name__ == '__main__':
	solve = Solve()
	tab=tabulate(solve.solution())
	print(tab)
	with open('../data/crossword_output.txt', 'w') as f:
		f.write(tab)
